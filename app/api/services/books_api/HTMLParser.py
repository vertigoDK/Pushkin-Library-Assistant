    ...
    def parse_records_table(self, soup: BeautifulSoup) -> List[Dict[str, str]]:
        ...
        for record in soup.find_all('table', class_='record'):
            ...
            if bo_cell:
                ...
                if bo_div:
                    record_data['type'] = bo_div.find(string=' Вид документа').next_element.strip() if bo_div.find(
                        string=' Вид документа') else ''
                    record_data['code'] = bo_div.find(string=' Шифр издания').next_element.strip() if bo_div.find(
                        string=' Шифр издания') else ''
                    record_data['author'] = bo_div.find(string=' Автор(ы)').next_element.strip() if bo_div.find(
                        string=' Автор(ы)') else ''
                    record_data['title'] = bo_div.find(string=' Заглавие').next_element.strip() if bo_div.find(
                        string=' Заглавие') else ''
                    record_data['parallel_title'] = bo_div.find(
                        string=' Параллельн. заглавия').next_element.strip() if bo_div.find(
                        string=' Параллельн. заглавия') else ''
                    record_data['publish_info'] = bo_div.find(
                        string=' Выходные данные').next_element.strip() if bo_div.find(
                        string=' Выходные данные') else ''
                    record_data['pages'] = bo_div.find(
                        string=' Колич.характеристики').next_element.strip() if bo_div.find(
                        string=' Колич.характеристики') else ''
                    record_data['series'] = bo_div.find(string=' Серия:').next_element.strip() if bo_div.find(
                        string=' Серия:') else ''
                    record_data['isbn_price'] = bo_div.find(
                        string=' ISBN,  Цена    ').next_element.strip() if bo_div.find(
                        string=' ISBN,  Цена    ') else ''
