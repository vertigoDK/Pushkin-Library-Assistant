...
class EngineIntentModel(BaseModel):
    books_engine: bool = Field(description="if user want to use books engine, for example книги абая")
    legends_engine: bool = Field(description="if user want to use legends engine, for example легенда о рахмановских ключах")

class EngineIntentRouter(LLMBase):

    def __init__(self):
        super().__init__()
        self._llm_prompt = "Тут промпт для модели написать"
